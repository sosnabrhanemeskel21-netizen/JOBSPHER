...
public class JobService {
    ...
    @Transactional
    public Job createJob(User employer, JobRequest request, String paymentProofPath) {
        ...
        job.setPaymentProofPath(paymentProofPath);
        job.setCompany(company);
        job.setStatus(Job.JobStatus.PENDING_APPROVAL);
        // job.setPublishedAt(LocalDateTime.now()); // Only set when approved
        ...
        notificationService.createNotification(
            ...
            "Your job posting '" + job.getTitle() + "' has been submitted and is waiting for admin approval.",
            "JOB_CREATED",
            "/jobs/" + savedJob.getId()
        );

        // Notify admins
        List<User> admins = companyService.getAdmins();
        for (User admin : admins) {
            notificationService.createNotification(
                admin,
                "New Job Pending Approval",
                "Employer " + employer.getEmail() + " posted a new job: " + job.getTitle() + ". Please review it.",
                "NEW_JOB_POSTED",
                "/admin/jobs"
            );
        }
    ...
    public Page<Job> searchJobs(String keyword, String category, String location,
        ...
        Page<Job> jobs = jobRepository.searchJobs(Job.JobStatus.ACTIVE, keyword, category, location, minSalary,
                maxSalary, pageable);
        // Relationships are eagerly fetched by EntityGraph, but ensure nested
        // company.employer is loaded
    ...
    public Job updateJob(Long jobId, User employer, JobRequest request) {
        ...
        // When updated, if it was rejected or active, we return it to pending for re-review
        job.setStatus(Job.JobStatus.PENDING_APPROVAL);
    ...
    @Transactional
    public Job closeJob(Long jobId, User employer) {
        Job job = getJobById(jobId);
