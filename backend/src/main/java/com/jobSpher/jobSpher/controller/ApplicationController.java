...
public class ApplicationController {
    ...
    public ResponseEntity<Application> createApplication(
        ...
        if (resume != null && !resume.isEmpty()) {
            ...
            // Using a more robust check in FileStorageService or here if needed
            // For now, rely on FileStorageService or allow permissive check
             if (!fileStorageService.isValidFileType(resume, ALLOWED_RESUME_TYPES)) {
                 System.out.println("Invalid file type: " + resume.getContentType());
     ...
     * 
     * Returns all applications for all jobs posted by the authenticated employer.
     * 
     * Required Role: EMPLOYER
     * 
     * @return ResponseEntity containing a List of Application entities
    ...
    public ResponseEntity<List<Application>> getApplicationsForEmployer() {
        User employer = getCurrentUserFromContext();
        List<Application> applications = applicationService.getApplicationsByEmployer(employer);
        return ResponseEntity.ok(applications);
    }
